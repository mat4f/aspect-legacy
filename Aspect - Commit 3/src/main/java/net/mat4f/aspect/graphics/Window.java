...
public class Window {
    ...
    public void asInitialize() {
        ...
        glfwSetKeyCallback(aspect_window_id, (window, key, scancode, action, mods) -> {
            if (action == GLFW_PRESS) {
                this.aspect_input_keyboard_keys[key] = true;
            } else if ( action == GLFW_RELEASE) {
                this.aspect_input_keyboard_keys[key] = false;
            }
        });
    ...
    public void render(Shader shader, GameObject object, Matrix4f view, Transformation transformation) {
        Matrix4f modelViewMatrix = transformation.getModelViewMatrix(object, view);
        shader.setUniform("view", modelViewMatrix);
